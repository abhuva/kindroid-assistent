...
try:
    ...
    if RUN_INTERVAL_MINUTES <= 0:
        logger.warning(f"Warning: run_interval_minutes must be positive. Using default ({DEFAULT_INTERVAL_MINUTES} minutes).")
        RUN_INTERVAL_MINUTES = DEFAULT_INTERVAL_MINUTES
except (ValueError, TypeError):
    logger.warning(f"Warning: Invalid run_interval_minutes. Using default ({DEFAULT_INTERVAL_MINUTES} minutes).")
    RUN_INTERVAL_MINUTES = DEFAULT_INTERVAL_MINUTES
...
try:
    ...
    if GEMINI_MAX_OUTPUT_TOKENS <= 0:
        logger.warning(f"Warning: gemini.max_output_tokens must be positive. Using default ({DEFAULT_MAX_TOKENS}).")
        GEMINI_MAX_OUTPUT_TOKENS = DEFAULT_MAX_TOKENS
except (ValueError, TypeError):
    logger.warning(f"Warning: Invalid gemini.max_output_tokens. Using default ({DEFAULT_MAX_TOKENS}).")
    GEMINI_MAX_OUTPUT_TOKENS = DEFAULT_MAX_TOKENS

try:
    MAX_RESPONSE_CHARS = config.get('gemini', {}).get('max_response_chars', DEFAULT_MAX_CHARS)
    if MAX_RESPONSE_CHARS <= 0:
        logger.warning(f"Warning: gemini.max_response_chars must be positive. Using default ({DEFAULT_MAX_CHARS}).")
        MAX_RESPONSE_CHARS = DEFAULT_MAX_CHARS
except (ValueError, TypeError):
    logger.warning(f"Warning: Invalid gemini.max_response_chars. Using default ({DEFAULT_MAX_CHARS}).")
    MAX_RESPONSE_CHARS = DEFAULT_MAX_CHARS
...
def send_message_to_kindroid(api_key, ai_id, message, call_description="Kindroid"):
    ...
    logger.info(f"Message Content: {message}") # Log complete message
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Received Text: {raw_text}") # Log complete response
            return raw_text
        else:
            ...
            try:
                ...
                if not candidates:
                    ...
                    if prompt_feedback and prompt_feedback.get('blockReason'):
                        ...
                        # Consider logging safety ratings: logger.info(f"Safety Ratings: {prompt_feedback.get('safetyRatings')}")
                        ...
                        logger.error(f"Raw Gemini response JSON: {json.dumps(response_data, indent=2)}")
                ...
                if parts:
                    ...
                    if generated_text is not None:
                         ...
                         logger.info(f"Received Text: {generated_text}") # Log complete response
                ...
                try:
                    logger.error(f"Raw Gemini response JSON: {json.dumps(response_data, indent=2)}")
                     ...
                     logger.error(f"Raw Gemini response: {response.text}") # Fallback if response_data wasn't created
                return None
        else:
...
def run_api_chain():
    ...
    if gemini_response.startswith("Error: Gemini request blocked"):
        logger.error(f"Gemini response indicates an error: '{gemini_response}'. Skipping final Kindroid call.")
        return # Stop execution for this run
    ...
    if len(gemini_response) > MAX_RESPONSE_CHARS:
        logger.info(f"  Truncating Gemini response to {MAX_RESPONSE_CHARS} characters.")
        truncated_gemini_response = gemini_response[:MAX_RESPONSE_CHARS]
        logger.info(f"  Truncated length: {len(truncated_gemini_response)} chars.")
    else:
        logger.info("  Gemini response within character limit. No truncation needed.")
